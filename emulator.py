...
class App:
            ...
            if self.port.in_waiting != 0:
                ...
                if mode == b'\x03':
                    ...
                    for i in range(0, bytes_written, 2):
                        self.registers[starting_register + i // 2] = self.data_packer.unpack(data[i: i + 2])[0]
    ...
    def register_entry_update(self):
        for i in range(0, len(self.register_entries)):
            if self.register_entries[i].get() != '' and self.registers[i] != int(self.register_entries[i].get()):
                self.register_entries[i].set(self.registers[i])
